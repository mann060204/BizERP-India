    ...
    const fetchData = async () => {
      try {
        ...
        const fetchUrl = isDev 
          ? `/backend-api/public/purchase-orders/${id}`
          : `https://bizerp-api.vercel.app/api/v1/public/purchase-orders/${id}`;
    ...
    <>
      ...
      <div className="bg-[#525659] min-h-screen text-black print:bg-white print:p-0 p-8 font-sans flex justify-center text-[11px]">
        ...
        <div className="border-2 border-gray-800 flex-1 flex flex-col">
          ...
          <div className="grid grid-cols-2 border-b-2 border-gray-800 text-[10px]">
            ...
            <div className="p-2 flex flex-col">
              <div className="font-bold text-blue-900 mb-1 border-b border-gray-300 pb-1">Purchase Order Details</div>
              <div className="grid grid-cols-[100px_1fr] gap-x-1 font-semibold text-gray-800">
                <span>Purchase Order No.</span><span>: {order.orderNumber}</span>
                <span>Purchase Order Date</span><span>: {new Date(order.orderDate).toLocaleDateString('en-GB')}</span>
                {order.dueDate && <><span>Valid Till</span><span>: {new Date(order.dueDate).toLocaleDateString('en-GB')}</span></>}
                <span>Payment Mode</span><span>: {order.paymentMode || 'N/A'}</span>
                {order.deliveryTerms && <><span>Transport</span><span className="whitespace-pre-wrap leading-tight">: {order.deliveryTerms}</span></>}
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-center border-collapse h-full text-[10px] table-fixed font-semibold text-gray-800">
              ...
              <tbody>
                ...
                <tr className="border-t-2 border-gray-800 bg-action-50 h-6 font-bold text-blue-900">
                  ...
                  {!isNonGst && <td className="border-r-2 border-gray-800"></td>}
                  <td className="border-r-2 border-gray-800">{order.lineItems.reduce((s:any, i:any)=>s + i.quantity, 0)}</td>
                  <td className="border-r-2 border-gray-800"></td>
                  {!isNonGst && <td className="border-r-2 border-gray-800 text-right pr-1">{order.totalTaxableAmount.toFixed(2)}</td>}
                  {!isNonGst && (
                    <td className="border-r-2 border-gray-800 p-0">
                       <div className="flex h-full">
                         <span className="w-1/2 border-r-2 border-gray-800"></span>
                         <span className="w-1/2 text-right pr-1 pt-[2px]">{order.totalGST.toFixed(2)}</span>
                       </div>
                    </td>
                  )}
                  <td className="text-right pr-1">{isNonGst ? order.subtotal.toFixed(2) : (order.totalTaxableAmount + order.totalGST).toFixed(2)}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div className="flex border-t-2 border-gray-800 text-[10px] font-bold min-h-[14rem] bg-white">
            ...
            <div className="w-[65%] flex flex-col border-r-2 border-gray-800">
               <div className="border-b-2 border-gray-800 px-2 py-1 flex justify-between bg-action-50 text-blue-900">
                 <span>Purchase Order Total in Words:</span>
                 <span className="italic uppercase">Rupees {numberToWords(Math.round(order.grandTotal))} Only</span>
               </div>
            ...
            <div className="w-[35%] flex flex-col font-bold text-gray-800">
               ...
               <div className="flex justify-between border-b border-gray-300 px-2 py-1">
                 <span>{isNonGst ? 'Subtotal :' : 'Taxable Amount :'}</span><span>{isNonGst ? order.subtotal.toFixed(2) : order.totalTaxableAmount.toFixed(2)}</span>
               </div>
               {!isNonGst && (
                 <>
                   {isInterState ? (
                     <div className="flex justify-between border-b border-gray-300 px-2 py-1">
                       <span>IGST Amt :</span><span>{order.totalIGST.toFixed(2)}</span>
                     </div>
                   ) : (
                     <>
                       <div className="flex justify-between border-b border-gray-300 px-2 py-1">
                         <span>CGST Amt :</span><span>{order.totalCGST.toFixed(2)}</span>
                       </div>
                       <div className="flex justify-between border-b border-gray-300 px-2 py-1">
                         <span>SGST Amt :</span><span>{order.totalSGST.toFixed(2)}</span>
                       </div>
                     </>
                   )}
                 </>
               ...
               <div className="flex justify-between border-b border-gray-300 px-2 py-1">
                 <span>Discount :</span><span className="text-red-600">-{order.totalDiscount.toFixed(2)}</span>
               </div>
               {order.shippingCharge > 0 && (
                 <div className="flex justify-between border-b border-gray-300 px-2 py-1">
                   <span>Shipping / Freight :</span><span>{order.shippingCharge.toFixed(2)}</span>
                 </div>
               )}
        ...
        <div className="text-center text-[10px] mt-1 text-gray-500 font-semibold print:text-gray-800">
          This is a computer generated Purchase Order — not a tax invoice.
        </div>
      ...
      <div className="print:hidden fixed bottom-8 right-8 flex flex-col gap-2">
        <button onClick={() => window.print()} className="px-6 py-3 bg-action-500 text-slate-900 rounded-full font-bold shadow-2xl hover:bg-action-600">Print Purchase Order</button>
