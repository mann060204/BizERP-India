...
import { suppliersApi, businessApi } from '../../../../../lib/erp-api';

export default function PrintableSupplierLedger() {
  const { id } = useParams();
    ...
    const fetchData = async () => {
      try {
        const [bizRes, custRes, ledgerRes] = await Promise.all([
          ...
          suppliersApi.get(id as string),
          suppliersApi.getLedger(id as string)
        ]);
  ...
  if (supplier?.__error) return <div className="p-10 text-center text-red-500 font-bold bg-white min-h-screen">Error: {supplier.__error}</div>;
  if (!supplier || !business) return <div className="p-10 text-center text-red-500 font-bold bg-white min-h-screen">Data not found</div>;
  ...
  let initialOpening = supplier.balanceType === 'Credit' ? -Math.abs(supplier.openingBalance) : Math.abs(supplier.openingBalance);
  ...
  const formatBal = (b: number) => {
    return formatAccountingBalance(b, 'supplier').text;
  };
  ...
  return (
    <>
      ...
      <div className="print:hidden fixed top-0 left-0 right-0 bg-slate-800 text-white p-3 flex justify-between items-center z-50 shadow-md">
        <span className="text-sm font-medium">{supplier.name} - Ledger Account</span>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              ...
              {displayTxns.map((txn, idx) => {
                ...
                else if (txn.referenceType === 'Payment') vchType = 'Payment';
              ...
              <tr className="font-bold border-b border-black">
                ...
                <td className="py-1 text-right pr-4 font-bold">{currentBal >= 0 ? <span className="text-red-600">Dr</span> : <span className="text-emerald-600">Cr</span>}</td>
