            ...
            <fieldset className="border border-slate-200 rounded bg-slate-50 p-4 relative pt-5 shadow-sm">
              ...
              <div className="space-y-3 text-xs">
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`font-bold ${formatAccountingBalance(currentBalance, 'customer').colorClass}`}>{formatAccountingBalance(currentBalance, 'customer').text}</span>
                </div>
                <div className="flex justify-between">
          ...
          <div className="flex-1 bg-slate-50 rounded border border-slate-200 p-4 shadow-sm min-h-0 overflow-y-auto">
                 ...
                 <div className={`grid grid-cols-1 lg:grid-cols-2 gap-4 ${isReadOnly ? 'pointer-events-none opacity-80' : ''}`}>
                   ...
                   <div className="space-y-4">
                     ...
                     <div className="flex justify-end gap-3 pt-4 pb-12 lg:pb-0">
                       <button onClick={() => router.push('/dashboard/customers')} className="px-6 py-2 rounded border border-slate-200 text-slate-600 hover:text-slate-900 hover:border-[#D4D4D4] text-sm font-medium transition pointer-events-auto">
               ...
               {activeTab === 'Invoices' && (
                 <div className="flex flex-col h-full">
                    <h3 className="font-semibold text-slate-900 mb-4">Customer Invoices</h3>
                    {invoices.length > 0 ? (
                      <div className="overflow-x-auto border border-slate-200 rounded-xl flex-1">
                        <table className="w-full text-sm text-left">
                          <thead className="bg-[#F1F5F9] text-slate-600 text-xs uppercase">
                            <tr>
                              <th className="px-4 py-3 border-b border-slate-200">Invoice No</th>
               ...
               {activeTab === 'Accounts' && (
                 <div className="flex flex-col h-full">
                    <h3 className="font-semibold text-slate-900 mb-4">Customer Ledger (Statement of Account)</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                      <div className="bg-[#F1F5F9] border border-slate-200 p-4 rounded-xl">
                         ...
                         <div className={`text-xl font-bold mt-1 ${formatAccountingBalance(currentBalance, 'customer').colorClass}`}>{formatAccountingBalance(currentBalance, 'customer').text}</div>
                      </div>
                      <div className="bg-[#F1F5F9] border border-slate-200 p-4 rounded-xl">
                          ...
                          <tbody className="divide-y divide-slate-100">
                            {(() => {
                              ...
                              const formatBal = (b: number | undefined) => {
                                ...
                                const { text, colorClass } = formatAccountingBalance(b, 'customer'); return <span className={`font-bold ${colorClass}`}>{text}</span>;
                              };
               ...
               {activeTab === 'Quotations' && (
                 <div className="flex flex-col h-full items-center justify-center text-slate-600 gap-3">
                    ...
                    <button className="mt-2 text-primary hover:text-blue-400 text-xs font-semibold underline">Create Quotation</button>
                 </div>
               )}
