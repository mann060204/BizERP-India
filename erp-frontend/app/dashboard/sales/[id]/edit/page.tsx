    ...
    <div className="flex flex-col h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
      ...
      <footer className="fixed bottom-0 left-0 right-0 h-12 bg-[#F1F5F9] border-t border-slate-200 flex items-center justify-between px-4 z-50">
        ...
        <div className="flex gap-2">
          <button onClick={() => handleUpdate('paid')} disabled={saving} className="bg-blue-600 hover:bg-blue-700 text-slate-900 px-4 py-1.5 rounded flex items-center gap-2 text-xs font-bold transition shadow-[0_0_15px_rgba(37,99,235,0.2)]">
            <Printer className="w-4 h-4" /> Save and Print
          </button>
          <button onClick={() => handleUpdate('sent')} disabled={saving} className="bg-blue-800 hover:bg-blue-900 text-slate-900 px-6 py-1.5 rounded flex items-center gap-2 text-xs font-bold transition">
