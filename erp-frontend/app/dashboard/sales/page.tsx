    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'This Month', value: `₹${(summary.monthSales || 0).toFixed(2)}`, sub: `${summary.monthInvoiceCount || 0} invoices`, color: 'dark:text-emerald-400 text-emerald-600', bg: 'bg-emerald-400/10' },
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <h2 className="text-xl font-bold dark:text-white text-gray-900">All Invoices</h2>
        ...
        {loading ? (
          ...
          <div className="glass rounded-2xl p-16 text-center">
            ...
            <p className="dark:text-white text-gray-900 font-semibold text-lg">No invoices yet</p>
            <p className="dark:text-[#475569] text-gray-500 text-sm mt-1 mb-6">Create your first GST invoice to get started</p>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-[#1A1A1A]">
                  {invoices.map((inv) => {
                    ...
                    return (
                      <tr key={inv._id} className="hover:dark:bg-[#111111] bg-gray-50 transition-colors group">
                        <td className="px-5 py-4 font-mono text-xs text-[#D4D4D4] font-semibold">{inv.invoiceNumber}</td>
                        <td className="px-5 py-4 dark:text-[#94a3b8] text-gray-600">{new Date(inv.invoiceDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</td>
                        <td className="px-5 py-4 dark:text-white text-gray-900 font-medium">{inv.customerSnapshot.name}</td>
                        <td className="px-5 py-4 dark:text-white text-gray-900 font-semibold">₹{inv.grandTotal.toFixed(2)}</td>
                        <td className="px-5 py-4 text-green-400">₹{inv.amountReceived.toFixed(2)}</td>
                        <td className="px-5 py-4"><span className={inv.balance > 0 ? 'text-red-400 font-medium' : 'dark:text-[#475569] text-gray-500'}>₹{inv.balance.toFixed(2)}</span></td>
                        <td className="px-5 py-4 dark:text-[#94a3b8] text-gray-600">{inv.paymentMode}</td>
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <Link href={`/print/invoice/${inv._id}`} target="_blank" className="p-1.5 rounded-lg dark:bg-[#1A1A1A] bg-gray-200 dark:text-[#94a3b8] text-gray-600 hover:dark:text-white text-gray-900 hover:bg-[#D4D4D4] transition tooltip" title="Print Invoice">
                              <Printer className="w-4 h-4" />
                            </Link>
                            {inv.status !== 'cancelled' && (
                              <Link href={`/dashboard/sales/${inv._id}/edit`} className="p-1.5 rounded-lg dark:bg-[#1A1A1A] bg-gray-200 dark:text-[#94a3b8] text-gray-600 hover:dark:text-white text-gray-900 hover:bg-blue-500 transition tooltip" title="Edit Invoice">
                                <Edit3 className="w-4 h-4" />
                              </Link>
                            )}
                            <button onClick={() => handleWhatsApp(inv)} className="p-1.5 rounded-lg dark:bg-[#1A1A1A] bg-gray-200 dark:text-[#94a3b8] text-gray-600 hover:dark:text-white text-gray-900 hover:bg-[#22c55e] transition" title="Share via WhatsApp">
                              <MessageCircle className="w-4 h-4" />
                            </button>
                            <button onClick={() => handleEmail(inv)} className="p-1.5 rounded-lg dark:bg-[#1A1A1A] bg-gray-200 dark:text-[#94a3b8] text-gray-600 hover:dark:text-white text-gray-900 hover:bg-[#D4D4D4] transition" title="Share via Email">
                              <Mail className="w-4 h-4" />
                            </button>
                            {inv.status !== 'cancelled' && (
                              <button onClick={() => handleCancel(inv._id, inv.invoiceNumber)} className="p-1.5 rounded-lg dark:bg-[#1A1A1A] bg-gray-200 dark:text-[#94a3b8] text-gray-600 hover:dark:text-white text-gray-900 hover:bg-red-500 transition" title="Cancel Invoice">
