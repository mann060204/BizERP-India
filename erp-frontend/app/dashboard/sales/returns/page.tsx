...
export default function SalesPage() {
  ...
  const handleCancel = async (id: string, num: string) => {
    if (!confirm(`Cancel salesReturn ${num}?`)) return;
    try { await salesReturnApi.cancel(id); toast.success('salesReturn cancelled'); setsalesReturns(inv => inv.filter(i => i._id !== id)); }
  ...
  const filteredsalesReturns = salesReturns.filter(inv => {
    if (!searchQuery) return true;
    const q = searchQuery.toLowerCase();
    return (
      inv.salesReturnNumber?.toLowerCase().includes(q) ||
      ...
      <main className="flex-1 p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/dashboard/sales/returns/new" className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary text-white hover:bg-primary-hover font-semibold text-sm hover:opacity-90 transition shadow-lg shadow-white/10/30 whitespace-nowrap">
        ...
        {loading ? (
          ...
          <div className="glass rounded-2xl p-16 text-center">
            ...
            <Link href="/dashboard/sales/returns/new" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-primary text-white hover:bg-primary-hover text-sm font-semibold hover:opacity-90 transition">
                      ...
                      <tr key={inv._id} className="hover:bg-[#F1F5F9] transition-colors group">
                          ...
                          <div className="flex flex-col gap-1 items-start">
                            ...
                            {inv.balance > 0 && inv.status !== 'cancelled' && inv.status !== 'paid' && (
                              <span className="text-[10px] font-bold text-orange-600 bg-orange-50 px-1.5 py-0.5 rounded border border-orange-200">
                                {Math.max(0, Math.floor((new Date().getTime() - new Date(inv.salesReturnDate).getTime()) / (1000 * 3600 * 24)))} days pending
                              </span>
                            )}
                          </div>
                        </td>
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <Link href={`/print/salesReturn/${inv._id}`} target="_blank" className="p-1.5 rounded-lg bg-[#E2E8F0] text-slate-600 hover:text-slate-900 hover:bg-[#D4D4D4] transition tooltip" title="Print salesReturn">
                              <Printer className="w-4 h-4" />
                            </Link>
                            {inv.status !== 'cancelled' && (
                              <Link href={`/dashboard/sales/${inv._id}/edit`} className="p-1.5 rounded-lg bg-[#E2E8F0] text-slate-600 hover:text-white hover:bg-primary transition tooltip" title="Edit salesReturn">
                            ...
                            {inv.status !== 'cancelled' && (
                              <button onClick={() => handleCancel(inv._id, inv.salesReturnNumber)} className="p-1.5 rounded-lg bg-[#E2E8F0] text-slate-600 hover:text-slate-900 hover:bg-red-500 transition" title="Cancel salesReturn">
