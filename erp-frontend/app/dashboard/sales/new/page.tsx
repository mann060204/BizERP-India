...
import { Plus, Trash2, Search, ChevronDown, Loader2, Save, CheckCircle } from 'lucide-react';
...
export default function NewInvoicePage() {
  ...
  useEffect(() => {
    customersApi.list({ limit: 200 }).then(r => setCustomers(r.data.customers)).catch(() => {});
  ...
  // Recalculate all lines when inter-state changes
  useEffect(() => {
  ...
  const handleSave = async (saveStatus: 'draft' | 'sent' | 'paid') => {
    ...
    } catch (e: any) { toast.error(e.response?.data?.message || 'Failed to save invoice'); }
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 p-6 space-y-6 max-w-6xl mx-auto w-full">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold text-white">Create GST Invoice</h2>
          <div className="flex gap-2">
            ...
            <button onClick={() => handleSave('paid')} disabled={saving} className="px-4 py-2 rounded-xl bg-white text-black hover:bg-gray-200 text-sm font-semibold transition hover:opacity-90 flex items-center gap-2 shadow-lg shadow-white/10/30">
              {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />} Save & Mark Paid
            </button>
          </div>
        </div>
          ...
          <div className="space-y-4">
            <div className="glass rounded-2xl p-5 space-y-3 sticky top-20">
              ...
              <button onClick={() => handleSave('sent')} disabled={saving} className="w-full py-3 rounded-xl bg-white text-black hover:bg-gray-200 font-semibold text-sm hover:opacity-90 disabled:opacity-60 transition flex items-center justify-center gap-2 shadow-lg shadow-white/10/30">
                {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />} Create Invoice
              </button>
            </div>
          </div>
        </div>
