...
const COUNTRY_CODES: Record<string, string> = {
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      await suppliersApi.delete(id as string);
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        email: form.email, phoneNo: form.phoneNo, mobile: form.mobile, mobileCode: form.mobileCode,
        contactPerson: form.contactPersonName, contactPersonNumber: form.contactPersonNumber, contactPersonCode: form.contactPersonCode,
  ...
  const handleAdjustmentSubmit = async () => {
    ...
    try {
      await suppliersApi.addAdjustment(id as string, adjustmentForm);
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      await suppliersApi.updateLedgerEntry(id as string, editForm.id, editForm);
  ...
  const handleDeleteEntry = async (entryId: string) => {
    ...
    try {
      await suppliersApi.deleteLedgerEntry(id as string, entryId);
                          ...
                          <tbody className="divide-y divide-[#1A1A1A]">
                            {purchaseBills.map((bill: any) => (
                               <tr key={bill._id} className="hover:bg-[#F1F5F9] cursor-pointer" onClick={() => router.push(`/dashboard/purchases`)}>
                                 ...
                                 <td className="px-4 py-3 text-right font-bold text-slate-900">₹{bill.grandTotal?.toFixed(3) || '0.00'}</td>
                               </tr>
                            ))}
                          </tbody>
                        </table>
                          ...
                          <tbody className="divide-y divide-[#1A1A1A]">
                            {ledger.filter((txn: any) => txn.referenceType === 'Payment' && txn.debit > 0).map((txn: any) => (
                               <tr key={txn._id} className="hover:bg-[#F1F5F9]">
                                 ...
                                 <td className="px-4 py-3 text-right font-bold text-emerald-500">+ ₹{txn.debit?.toFixed(3)}</td>
                               </tr>
                            ))}
                          </tbody>
                        </table>
                 ...
                 <div className="flex flex-col h-full">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                      ...
                      <div className="bg-[#F1F5F9] border border-slate-200 p-4 rounded-xl">
                         ...
                         <div className="text-xl font-bold mt-1 text-orange-400">₹{purchaseBills.reduce((acc, inv) => acc + (inv.grandTotal || 0), 0).toFixed(3)}</div>
                      </div>
                      <div className="bg-[#F1F5F9] border border-slate-200 p-4 rounded-xl">
                         <div className="text-xs text-slate-600 uppercase">Total Paid</div>
                         <div className="text-xl font-bold mt-1 text-emerald-400">₹{purchaseBills.reduce((acc, inv) => acc + (inv.amountPaid || 0), 0).toFixed(3)}</div>
                      </div>
                    </div>
                          ...
                          <tbody className="divide-y divide-slate-100">
                            {(() => {
                              ...
                              const formatBal = (b: number | undefined) => {
                                ...
                                const { text, colorClass } = formatAccountingBalance(b, 'supplier'); return <span className={`font-bold ${colorClass}`}>{text}</span>;
                              };
