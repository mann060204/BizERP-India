          ...
          {!selected ? (
            ...
            <>
                      ...
                      <tbody className="divide-y divide-slate-50">
                        {invoices.slice(0, 10).map((inv: any, i: number) => (
                          <tr key={i} className="hover:bg-slate-50">
                            ...
                            <td className="py-2 text-right text-emerald-600">{INR(inv.paidAmount)}</td>
                            <td className="py-2 text-right text-amber-600">{INR(inv.grandTotal - inv.paidAmount)}</td>
                            <td className="py-2 text-center"><span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${inv.paymentStatus === 'PAID' ? 'bg-emerald-50 text-emerald-700' : inv.paymentStatus === 'PARTIAL' ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-700'}`}>{inv.paymentStatus}</span></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              {tab === 'Purchase History' && (
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                  <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50"><h3 className="font-semibold text-slate-800">All Invoices ({invoices.length})</h3></div>
                  <table className="w-full text-sm">
                    <thead><tr className="text-xs text-slate-400 uppercase bg-slate-50 border-b">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {invoices.length === 0 ? (
                        ...
                        <tr key={i} className="hover:bg-slate-50">
                          ...
                          <td className="px-4 py-2.5 text-right text-emerald-600">{INR(inv.paidAmount)}</td>
                          <td className="px-4 py-2.5 text-right text-amber-600">{INR(inv.grandTotal - inv.paidAmount)}</td>
                          <td className="px-4 py-2.5 text-center"><span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${inv.paymentStatus === 'PAID' ? 'bg-emerald-50 text-emerald-700' : inv.paymentStatus === 'PARTIAL' ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-700'}`}>{inv.paymentStatus}</span></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
                  ...
                  <div className="space-y-3">
                    {(() => {
                      const now = Date.now();
                      const buckets = [
                        { label: 'Current (0–30 days)', invs: invoices.filter(i => i.paymentStatus !== 'PAID' && (now - new Date(i.invoiceDate).getTime()) <= 30 * 86400000) },
                        { label: '31–60 days', invs: invoices.filter(i => i.paymentStatus !== 'PAID' && (now - new Date(i.invoiceDate).getTime()) > 30 * 86400000 && (now - new Date(i.invoiceDate).getTime()) <= 60 * 86400000) },
                        { label: '61–90 days', invs: invoices.filter(i => i.paymentStatus !== 'PAID' && (now - new Date(i.invoiceDate).getTime()) > 60 * 86400000 && (now - new Date(i.invoiceDate).getTime()) <= 90 * 86400000) },
                        { label: '91–180 days', invs: invoices.filter(i => i.paymentStatus !== 'PAID' && (now - new Date(i.invoiceDate).getTime()) > 90 * 86400000 && (now - new Date(i.invoiceDate).getTime()) <= 180 * 86400000) },
                        { label: '180+ days (Critical)', invs: invoices.filter(i => i.paymentStatus !== 'PAID' && (now - new Date(i.invoiceDate).getTime()) > 180 * 86400000) },
                      ];
