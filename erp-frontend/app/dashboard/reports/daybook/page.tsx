      ...
      <main className="flex-1 p-6 space-y-6 max-w-5xl mx-auto w-full">
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
          <div>
            ...
            <p className="text-slate-600 text-sm mt-0.5">Chronological record of all inflows and outflows</p>
          </div>
          <input type="date" value={targetDate} onChange={e => setTargetDate(e.target.value)}
            className="px-4 py-2.5 rounded-xl bg-white border border-slate-200 text-slate-900 focus:outline-none focus:border-[#D4D4D4] transition" />
        </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="glass rounded-2xl p-5 border border-slate-200">
                <p className="text-slate-600 text-xs font-semibold uppercase tracking-wider mb-2">Total Receipts (Inflow)</p>
                <p className="text-2xl font-bold text-green-400">₹{totalInflow.toFixed(2)}</p>
              </div>
              <div className="glass rounded-2xl p-5 border border-slate-200">
                <p className="text-slate-600 text-xs font-semibold uppercase tracking-wider mb-2">Total Payments (Outflow)</p>
                <p className="text-2xl font-bold text-red-400">₹{totalOutflow.toFixed(2)}</p>
              </div>
              <div className="glass rounded-2xl p-5 border border-slate-200 bg-gradient-to-r from-blue-500/10 to-transparent">
                      ...
                      {data.transactions.map((t: any, idx: number) => {
                        ...
                        return (
                          <tr key={idx} className="hover:bg-[#F1F5F9] transition-colors">
                            ...
                            <td className="px-5 py-4 text-slate-600">{t.mode}</td>
