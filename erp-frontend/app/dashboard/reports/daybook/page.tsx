    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 p-6 space-y-6 max-w-5xl mx-auto w-full">
        <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
          <div>
            <h2 className="text-xl font-bold dark:text-white text-gray-900">Daily Transaction Ledger</h2>
            <p className="dark:text-[#94a3b8] text-gray-600 text-sm mt-0.5">Chronological record of all inflows and outflows</p>
          </div>
          <input type="date" value={targetDate} onChange={e => setTargetDate(e.target.value)}
            className="px-4 py-2.5 rounded-xl dark:bg-[#0A0A0A] bg-white border dark:border-[#1A1A1A] border-gray-300 dark:text-white text-gray-900 focus:outline-none focus:border-[#D4D4D4] transition" />
        </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="glass rounded-2xl p-5 border dark:border-[#1A1A1A] border-gray-300">
                <p className="dark:text-[#94a3b8] text-gray-600 text-xs font-semibold uppercase tracking-wider mb-2">Total Receipts (Inflow)</p>
                <p className="text-2xl font-bold text-green-400">₹{totalInflow.toFixed(2)}</p>
              </div>
              <div className="glass rounded-2xl p-5 border dark:border-[#1A1A1A] border-gray-300">
                <p className="dark:text-[#94a3b8] text-gray-600 text-xs font-semibold uppercase tracking-wider mb-2">Total Payments (Outflow)</p>
                <p className="text-2xl font-bold text-red-400">₹{totalOutflow.toFixed(2)}</p>
              </div>
              <div className="glass rounded-2xl p-5 border dark:border-[#1A1A1A] border-gray-300 bg-gradient-to-r from-blue-500/10 to-transparent">
                <p className="dark:text-[#94a3b8] text-gray-600 text-xs font-semibold uppercase tracking-wider mb-2">Net Cash Flow</p>
            ...
            {data.transactions.length === 0 ? (
              <div className="glass rounded-2xl p-16 text-center">
                ...
                <p className="dark:text-white text-gray-900 font-semibold text-lg">No transactions on this date</p>
              </div>
            ) : (
              <div className="glass rounded-2xl overflow-hidden border dark:border-[#1A1A1A] border-gray-300">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="dark:bg-[#0A0A0A] bg-white border-b dark:border-[#1A1A1A] border-gray-300">
                    ...
                    <tbody className="divide-y divide-[#1A1A1A]">
                      {data.transactions.map((t: any, idx: number) => {
                        ...
                        return (
                          <tr key={idx} className="hover:dark:bg-[#111111] bg-gray-50 transition-colors">
                            ...
                            <td className="px-5 py-4 dark:text-white text-gray-900 font-mono text-xs">{t.ref}</td>
                            <td className="px-5 py-4 dark:text-white text-gray-900 font-medium">{t.party}</td>
                            <td className="px-5 py-4 dark:text-[#94a3b8] text-gray-600">{t.mode}</td>
