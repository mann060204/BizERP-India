    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 p-6 space-y-6 max-w-5xl mx-auto w-full">
        <div>
          ...
          <p className="dark:text-[#94a3b8] text-gray-600 text-sm mt-1">Upload CSV files to securely add hundreds of records at once.</p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-1 space-y-6">
            {/* Import Type */}
            <div className="glass rounded-2xl p-6 border dark:border-[#1A1A1A] border-gray-300">
              <label className="block text-xs font-medium dark:text-[#94a3b8] text-gray-600 mb-3 uppercase tracking-wider">Select Entity to Import</label>
              <div className="flex flex-col gap-3">
                <button onClick={() => { setImportType('products'); setFile(null); setData([]); }} 
                  className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${importType === 'products' ? 'bg-[#D4D4D4]/10 border-[#D4D4D4] text-[#D4D4D4]' : 'dark:bg-[#0A0A0A] bg-white dark:border-[#1A1A1A] border-gray-300 dark:text-white text-gray-900 hover:border-[#475569]'}`}>
                ...
                <button onClick={() => { setImportType('customers'); setFile(null); setData([]); }} 
                  className={`flex items-center gap-3 p-3 rounded-xl border transition-all ${importType === 'customers' ? 'bg-[#D4D4D4]/10 border-[#D4D4D4] text-[#D4D4D4]' : 'dark:bg-[#0A0A0A] bg-white dark:border-[#1A1A1A] border-gray-300 dark:text-white text-gray-900 hover:border-[#475569]'}`}>
            ...
            {/* Formatting Info */}
            <div className="glass rounded-2xl p-6 border dark:border-[#1A1A1A] border-gray-300 bg-blue-500/5">
              <div className="flex items-start gap-3">
                <Info className="w-5 h-5 text-blue-400 mt-0.5" />
                <div>
                  <h4 className="dark:text-white text-gray-900 font-medium text-sm mb-1">CSV Formatting Guide</h4>
                  <p className="dark:text-[#94a3b8] text-gray-600 text-xs leading-relaxed mb-3">Ensure your CSV contains column headers in the first row. The system will automatically attempt to match common header names.</p>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="glass rounded-2xl p-8 border border-dashed border-[#475569] flex flex-col items-center justify-center min-h-[250px] transition hover:border-[#D4D4D4] hover:bg-[#D4D4D4]/5 relative group cursor-pointer"
              ...
              {!file ? (
                <>
                  <div className="w-16 h-16 rounded-full dark:bg-[#0A0A0A] bg-white flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                    <FileText className="w-8 h-8 dark:text-[#94a3b8] text-gray-600 group-hover:text-[#D4D4D4] transition-colors" />
                  </div>
                  <h3 className="dark:text-white text-gray-900 font-semibold text-lg mb-1">Click to Upload CSV</h3>
                  <p className="dark:text-[#94a3b8] text-gray-600 text-sm">or drag and drop here</p>
                </>
              ) : (
            ...
            {data.length > 0 && (
              <div className="glass rounded-2xl p-6 border dark:border-[#1A1A1A] border-gray-300">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="dark:text-white text-gray-900 font-semibold text-sm uppercase tracking-wider">Data Preview (Top 3)</h3>
                ...
                <div className="overflow-x-auto rounded-xl border dark:border-[#1A1A1A] border-gray-300">
                  <table className="w-full text-left text-sm dark:text-[#94a3b8] text-gray-600">
                    <thead className="dark:bg-[#0A0A0A] bg-white dark:text-white text-gray-900">
                      <tr>
                    ...
                    <tbody className="divide-y divide-[#1A1A1A]">
                      {data.slice(0, 3).map((row, i) => (
                        <tr key={i} className="hover:dark:bg-[#0A0A0A] bg-white/50">
                  ...
                  {data.length > 3 && (
                    <div className="p-3 text-center text-xs dark:text-[#475569] text-gray-500 dark:bg-[#0A0A0A] bg-white/30">
