  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 p-6 space-y-6 max-w-6xl mx-auto w-full">
        <div>
          <h2 className="text-xl font-bold dark:text-white text-gray-900">Tools & Utilities</h2>
          <p className="dark:text-[#94a3b8] text-gray-600 text-sm mt-1">Additional modules to streamline your operations.</p>
        </div>
          ...
          {TOOLS.map(tool => {
            ...
            const Card = (
              <div className={`glass ngrok-card-hover rounded-2xl p-6 transition-all duration-300 ${isLive ? 'cursor-pointer group' : 'opacity-60 cursor-not-allowed'}`}>
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 transition ${isLive ? 'bg-[#D4D4D4]/10 text-[#D4D4D4] group-hover:bg-[#D4D4D4] group-hover:dark:text-white text-gray-900' : 'dark:bg-[#0A0A0A] bg-white dark:text-[#94a3b8] text-gray-600'}`}>
