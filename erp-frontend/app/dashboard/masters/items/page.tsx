...
const Input = ({ label, required = false, type = 'text', keyName, form, setForm, placeholder = '' }: any) => (
  <div>
    <label className="block text-[11px] font-medium dark:text-[#94a3b8] text-gray-600 mb-1 uppercase tracking-wider">{label} {required && <span className="text-red-500">*</span>}</label>
    <input type={type}
      ...
      className="w-full px-3 py-2 rounded-lg dark:bg-[#111111] bg-gray-50 border dark:border-[#1A1A1A] border-gray-300 dark:text-white text-gray-900 placeholder-[#475569] focus:outline-none focus:border-[#D4D4D4] text-sm transition" />
  </div>
);
...
const Select = ({ label, required = false, keyName, form, setForm, options }: any) => (
  <div>
    ...
    <select value={form[keyName]} onChange={e => setForm({ ...form, [keyName]: e.target.value })}
      className="w-full px-3 py-2 rounded-lg dark:bg-[#111111] bg-gray-50 border dark:border-[#1A1A1A] border-gray-300 dark:text-white text-gray-900 focus:outline-none focus:border-[#D4D4D4] text-sm transition appearance-none">
...
const Checkbox = ({ label, keyName, form, setForm, danger = false }: any) => (
  <label className={`flex items-center gap-2 text-sm cursor-pointer ${danger ? 'text-red-400 font-medium' : 'dark:text-white text-gray-900'}`}>
    <input type="checkbox" checked={form[keyName]} onChange={e => setForm({ ...form, [keyName]: e.target.checked })}
      className="w-4 h-4 rounded dark:border-[#1A1A1A] border-gray-300 dark:bg-[#111111] bg-gray-50 text-blue-500 focus:ring-blue-500 focus:ring-offset-black" />
    {label}
  </label>
);
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold dark:text-white text-gray-900">Items & Services</h2>
            <p className="dark:text-[#94a3b8] text-gray-600 text-sm mt-0.5">{products.length} item{products.length !== 1 ? 's' : ''} in master</p>
          </div>
        ...
        <div className="flex gap-3 flex-wrap">
          <div className="relative flex-1 min-w-52">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 dark:text-[#475569] text-gray-500" />
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search items..."
              className="w-full pl-9 pr-4 py-2.5 rounded-xl dark:bg-[#0A0A0A] bg-white border dark:border-[#1A1A1A] border-gray-300 dark:text-white text-gray-900 placeholder-[#475569] focus:outline-none focus:border-[#D4D4D4] transition text-sm" />
          </div>
          {['', 'product', 'service'].map(t => (
            <button key={t} onClick={() => setTypeFilter(t)}
              className={`px-4 py-2 rounded-xl text-sm font-medium border transition ${typeFilter === t ? 'bg-white text-black hover:bg-gray-200 border-transparent' : 'dark:border-[#1A1A1A] border-gray-300 dark:text-[#94a3b8] text-gray-600 hover:dark:text-white text-gray-900 hover:border-[#D4D4D4]'}`}>
        ...
        {loading ? (
          ...
          <div className="glass rounded-2xl p-16 text-center">
            ...
            <p className="dark:text-white text-gray-900 font-semibold text-lg">No items yet</p>
            <p className="dark:text-[#475569] text-gray-500 text-sm mt-1 mb-6">Add your products and services to start billing</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-[#1A1A1A]">
                  {products.map((p) => (
                    <tr key={p._id} className="hover:dark:bg-[#111111] bg-gray-50 transition-colors group">
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="dark:text-white text-gray-900 font-medium">{p.name}</p>
                            {p.sku && <p className="dark:text-[#475569] text-gray-500 text-xs font-mono">{p.sku}</p>}
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-4 dark:text-[#94a3b8] text-gray-600 text-xs">
                        {p.group && <div className="dark:text-white text-gray-900">{p.group}</div>}
                        {p.brand && <div>{p.brand}</div>}
                        {!p.group && !p.brand && '—'}
                      </td>
                      <td className="px-5 py-4 dark:text-[#94a3b8] text-gray-600">
                        <span className="dark:text-white text-gray-900">{p.unit}</span>
                        {p.secondaryUnit && <div className="text-xs">1 = {p.conversionRate} {p.secondaryUnit}</div>}
                      </td>
                      <td className="px-5 py-4 dark:text-[#94a3b8] text-gray-600">₹{p.purchasePrice.toFixed(2)}</td>
                      <td className="px-5 py-4 dark:text-white text-gray-900 font-semibold">₹{p.sellingPrice.toFixed(2)}</td>
                      <td className="px-5 py-4 dark:text-[#94a3b8] text-gray-600">{p.mrp ? `₹${p.mrp.toFixed(2)}` : '—'}</td>
                      ...
                      <td className="px-5 py-4">
                        {p.type === 'product' ? (
                          <span className={p.currentStock <= p.reorderLevel ? 'text-red-400 font-medium' : 'dark:text-emerald-400 text-emerald-600'}>
                            {p.currentStock} {p.unit}
                          </span>
                        ) : <span className="dark:text-[#475569] text-gray-500">N/A</span>}
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => openEdit(p)} className="p-1.5 rounded-lg hover:dark:bg-[#1A1A1A] bg-gray-200 dark:text-[#94a3b8] text-gray-600 hover:dark:text-white text-gray-900 transition"><Edit2 className="w-4 h-4" /></button>
                          <button onClick={() => handleDelete(p._id, p.name)} className="p-1.5 rounded-lg hover:bg-red-900/20 dark:text-[#94a3b8] text-gray-600 hover:text-red-400 transition"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 dark:bg-black bg-gray-50/60 backdrop-blur-sm">
          <div className="dark:bg-[#050505] bg-gray-100 border dark:border-[#1A1A1A] border-gray-300 rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col h-[600px]">
            
            <div className="flex items-center justify-between p-6 border-b dark:border-[#1A1A1A] border-gray-300 shrink-0">
              <div>
                <h3 className="dark:text-white text-gray-900 font-bold text-xl">{editing ? 'Edit Item' : 'Add New Item'}</h3>
                <p className="text-sm dark:text-[#94a3b8] text-gray-600 mt-1">Fill in the product details below</p>
              </div>
              <button onClick={() => setShowModal(false)} className="p-2 rounded-xl hover:dark:bg-[#111111] bg-gray-50 dark:text-[#94a3b8] text-gray-600 hover:dark:text-white text-gray-900 transition"><X className="w-5 h-5" /></button>
            </div>

            <div className="flex flex-1 overflow-hidden">
              {/* Sidebar Tabs */}
              <div className="w-48 border-r dark:border-[#1A1A1A] border-gray-300 p-4 space-y-1 overflow-y-auto dark:bg-[#0A0A0A] bg-white">
                {tabs.map(tab => (
                  <button
                    key={tab.id}
                    onClick={() => setActiveTab(tab.id)}
                    className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm font-medium transition ${activeTab === tab.id ? 'bg-white text-black' : 'dark:text-[#94a3b8] text-gray-600 hover:dark:bg-[#111111] bg-gray-50 hover:dark:text-white text-gray-900'}`}
                  >
              ...
              <div className="flex-1 p-6 overflow-y-auto">
                {activeTab === 'basic' && (
                  <div className="space-y-4">
                    <h4 className="text-sm font-semibold dark:text-white text-gray-900 mb-2 border-b dark:border-[#1A1A1A] border-gray-300 pb-2">Basic Details</h4>
                ...
                {activeTab === 'pricing' && (
                  <div className="space-y-4">
                    <h4 className="text-sm font-semibold dark:text-white text-gray-900 mb-2 border-b dark:border-[#1A1A1A] border-gray-300 pb-2">Pricing</h4>
                    <div className="grid grid-cols-2 gap-4">
                ...
                {activeTab === 'stock' && (
                  <div className="space-y-4">
                    <h4 className="text-sm font-semibold dark:text-white text-gray-900 mb-2 border-b dark:border-[#1A1A1A] border-gray-300 pb-2">Stock & Units</h4>
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      {form.secondaryUnit && form.secondaryUnit !== form.unit && (
                        <div className="col-span-2 p-3 dark:bg-[#111111] bg-gray-50 border border-[#1e3a8a]/30 rounded-lg flex items-center justify-between">
                          <span className="text-sm dark:text-[#94a3b8] text-gray-600">1 {form.unit} equals to:</span>
                          <div className="flex items-center gap-2">
                            <input type="number" value={form.conversionRate || 1} onChange={e => setForm({...form, conversionRate: parseFloat(e.target.value) || 1})} 
                              className="w-20 px-2 py-1.5 rounded-md dark:bg-black bg-gray-50 border dark:border-[#1A1A1A] border-gray-300 dark:text-white text-gray-900 text-center text-sm focus:border-[#D4D4D4] outline-none" />
                            <span className="text-sm dark:text-white text-gray-900 font-medium">{form.secondaryUnit}</span>
                          </div>
                        </div>
                      )}
                ...
                {activeTab === 'tax' && (
                  <div className="space-y-4">
                    <h4 className="text-sm font-semibold dark:text-white text-gray-900 mb-2 border-b dark:border-[#1A1A1A] border-gray-300 pb-2">Tax & Location</h4>
                    <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-[11px] font-medium dark:text-[#94a3b8] text-gray-600 mb-1 uppercase tracking-wider">Description</label>
                      <textarea value={form.description} onChange={e => setForm({...form, description: e.target.value})} rows={3}
                        className="w-full px-3 py-2 rounded-lg dark:bg-[#111111] bg-gray-50 border dark:border-[#1A1A1A] border-gray-300 dark:text-white text-gray-900 placeholder-[#475569] focus:outline-none focus:border-[#D4D4D4] text-sm transition resize-none" />
                    </div>
                  </div>
                )}
                ...
                {activeTab === 'settings' && (
                  <div className="space-y-4">
                    <h4 className="text-sm font-semibold dark:text-white text-gray-900 mb-2 border-b dark:border-[#1A1A1A] border-gray-300 pb-2">Item Settings</h4>
                    <div className="grid grid-cols-1 gap-y-4 dark:bg-[#111111] bg-gray-50 border dark:border-[#1A1A1A] border-gray-300 rounded-xl p-5">
                      ...
                      <div className="pt-2 border-t dark:border-[#1A1A1A] border-gray-300">
            ...
            <div className="flex gap-3 p-6 border-t dark:border-[#1A1A1A] border-gray-300 dark:bg-[#0A0A0A] bg-white shrink-0 rounded-b-2xl">
              <button onClick={() => setShowModal(false)} className="px-6 py-2.5 rounded-xl border dark:border-[#1A1A1A] border-gray-300 dark:text-[#94a3b8] text-gray-600 hover:dark:text-white text-gray-900 hover:border-[#D4D4D4] font-medium text-sm transition">
                Cancel
              </button>
