...
export default function EditPurchaseOrderPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (id) {
          ...
          setorderDate(new Date(pur.orderDate).toISOString().split('T')[0]);
          if (pur.dueDate) setDueDate(new Date(pur.dueDate).toISOString().split('T')[0]);
          setPlaceOfSupply(pur.placeOfSupply || 'Gujarat');
          setorderNumber(pur.orderNumber || '');
          setEwayBillNo(pur.ewayBillNo || '');
          setPaymentTerms(pur.paymentTerms || '');
          ...
          if (pur.supplierId) {
            ...
            if (pur.supplierId.address) {
              if (typeof pur.supplierId.address === 'string') {
                addrStr = pur.supplierId.address;
              } else {
                const parts = [
                  pur.supplierId.address.street,
                  pur.supplierId.address.city,
                  pur.supplierId.address.state,
                  pur.supplierId.address.pinCode,
                  pur.supplierId.address.country
                ].filter(Boolean);
                addrStr = parts.join(', ');
              }
            }
          ...
          setBatches(pur.batches || []);
          
          setAdditionalDiscount(pur.additionalDiscount || 0);
          setShippingCharge(pur.shippingCharge || 0);
          setShippingGstRate(pur.shippingGstRate || 0);
          setRemarks(pur.notes || '');
          
          setPaymentMode(pur.paymentMode || 'Cash');
  ...
  const handleSave = async (saveStatus: string) => {
    ...
    try {
      ...
      toast.success(`Purchase Order ${orderNumber} Updated!`);
      router.push('/dashboard/purchases/orders');
    } catch (e: any) {
      toast.error(e.response?.data?.message || 'Failed to save purchase order');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
      <Topbar title="Edit Purchase Order" />

      <div className="flex px-4 pt-2 border-b border-slate-200 bg-[#F1F5F9] gap-1">
         <div onClick={() => setActiveTab('bill')} className={`px-4 py-2 text-xs font-semibold cursor-pointer ${activeTab === 'bill' ? 'bg-[#F1F5F9] border border-b-0 border-slate-200 rounded-t text-slate-900' : 'text-slate-600 hover:text-slate-900 border border-transparent'}`}>Purchase Order</div>
      ...
      <main className="flex-1 overflow-y-auto p-1 space-y-1 pb-14 bg-slate-50">
        ...
        <div className="erp-container">
          <div className="erp-header py-1 text-xs">Purchase order information</div>
          <div className="p-1.5 grid grid-cols-5 gap-x-2 gap-y-1">
            ...
            <div>
              <label className="erp-label block mb-1">Order Date</label>
              <input type="date" value={orderDate} onChange={e => setorderDate(e.target.value)} className="erp-input w-full" />
            </div>
            <div>
            ...
            <div>
              <label className="erp-label block mb-1">Order No.</label>
              <input value={orderNumber} onChange={e => setorderNumber(e.target.value)} className="erp-input w-full" />
            </div>
            <div>
           ...
           <div className="col-span-1 space-y-2">
              ...
              {lastPrices.length > 0 && (
                <div className="mt-4 w-full p-2 bg-blue-50 border border-blue-100 rounded text-[10px] text-blue-800">
                  <p className="font-bold mb-1 border-b border-blue-200 pb-1">Last 5 purchases/orders ({itemInput.productName})</p>
      ...
      <footer className="fixed bottom-0 left-0 right-0 h-12 bg-[#F1F5F9] border-t border-slate-200 flex items-center justify-between px-4 z-50">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => handleSave('Sent')} disabled={saving} className="bg-action-700 hover:bg-action-800 text-white px-6 py-1.5 rounded flex items-center gap-2 text-xs font-bold transition">
              <Save className="w-4 h-4" /> Update & Mark Sent
            </button>
          </div>
      </footer>
    </div>
