...
export default function NewPurchaseOrderPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [sRes, pRes, bRes, nextNo] = await Promise.all([
        ...
        setorderNumber(nextNo.nextPurchaseOrderNumber);
  ...
  const handleSave = async (saveStatus: 'draft' | 'received' | 'paid') => {
    ...
    try {
      ...
      toast.success(`Purchase Order ${orderNumber} Recorded!`);
              ...
              {lineItems.length === 0 ? (
                ...
                lineItems.map((item, idx) => (
                  <div key={idx} className={`grid ${ purchaseType !== 'Non-GST' ? 'grid-cols-13' : 'grid-cols-10' } hover:bg-slate-50 border-b border-slate-100 text-[11px] group`}>
                    ...
                    <div className="border-r border-slate-100 px-2 py-1.5 text-right">₹{(item.rate || 0).toFixed(3)}</div>
      ...
      <footer className="fixed bottom-0 left-0 right-0 h-12 bg-[#F1F5F9] border-t border-slate-200 flex items-center justify-between px-4 z-50">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => handleSave('Sent')} disabled={saving} className="bg-action-700 hover:bg-action-800 text-white px-6 py-1.5 rounded flex items-center gap-2 text-xs font-bold transition">
              <Save className="w-4 h-4" /> Save & Mark Sent
            </button>
          </div>
      </footer>
    </div>
