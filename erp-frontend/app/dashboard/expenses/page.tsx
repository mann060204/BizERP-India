    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="glass rounded-2xl p-6 lg:col-span-1 flex flex-col justify-center">
            <p className="text-[#64748B] text-sm font-medium uppercase tracking-wider mb-2">Total Expenses (This Month)</p>
          ...
          <div className="glass rounded-2xl p-5 lg:col-span-2 overflow-x-auto whitespace-nowrap space-x-4 flex items-center">
            {summary.byCategory && summary.byCategory.length > 0 ? (
              summary.byCategory.map((c: any) => (
                <div key={c._id} className="inline-block bg-white border border-[#E2E8F0] rounded-xl p-3 min-w-[140px]">
                  <p className="text-[#64748B] text-xs font-medium truncate">{c._id}</p>
                  <p className="text-[#0F172A] font-bold mt-1">₹{c.total.toFixed(2)}</p>
                </div>
              ))
            ) : (
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="relative max-w-sm flex-1">
            ...
            <select value={search} onChange={e => setSearch(e.target.value)}
              className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-white border border-[#E2E8F0] text-[#0F172A] focus:outline-none focus:border-[#D4D4D4] transition text-sm appearance-none">
        ...
        {loading ? (
          ...
          <div className="glass rounded-2xl p-16 text-center">
            ...
            <p className="text-[#0F172A] font-semibold text-lg">No expenses found</p>
                ...
                <tbody className="divide-y divide-[#1A1A1A]">
                  {expenses.map((e) => (
                    <tr key={e._id} className="hover:bg-[#F1F5F9] transition-colors group">
                      <td className="px-5 py-4 text-[#64748B]">{new Date(e.date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}</td>
                      <td className="px-5 py-4"><span className="px-2 py-1 bg-white border border-[#E2E8F0] rounded-lg text-[#0F172A] text-xs">{e.category}</span></td>
                      <td className="px-5 py-4">
                        <p className="text-[#0F172A] font-medium">{e.vendorName || '—'}</p>
                        {e.notes && <p className="text-[#475569] text-xs mt-0.5 max-w-[200px] truncate">{e.notes}</p>}
                      </td>
                      <td className="px-5 py-4 text-[#64748B]">{e.paymentMode}</td>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#F8FAFC] backdrop-blur-sm">
          <div className="bg-white border border-[#E2E8F0] rounded-2xl w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-[#E2E8F0]">
              <h3 className="text-[#0F172A] font-bold text-lg">Record Expense</h3>
            ...
            <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-[#64748B] mb-1.5">Payment Mode</label>
              ...
              <div>
                <label className="block text-xs font-medium text-[#64748B] mb-1.5">Vendor / Payee Name</label>
                <input value={form.vendorName} onChange={e => setForm({ ...form, vendorName: e.target.value })} placeholder="e.g. BSNL, Reliance, John Doe"
                  className="w-full px-3 py-2.5 rounded-lg bg-[#F1F5F9] border border-[#E2E8F0] text-[#0F172A] placeholder-[#475569] focus:outline-none focus:border-[#D4D4D4] text-sm transition" />
              </div>
              
              <div className="p-4 bg-[#F8FAFC] rounded-xl border border-[#E2E8F0] space-y-3">
                <p className="text-xs font-semibold text-[#0F172A] uppercase tracking-wider">Tax Details (Optional)</p>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-[#64748B] mb-1.5">GST %</label>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-[#64748B] mb-1.5">Supply Type</label>
                    <div className="flex rounded-lg overflow-hidden border border-[#E2E8F0]">
                      {[{ label: 'Intra (C+S)', v: false }, { label: 'Inter (I)', v: true }].map(({ label, v }) => (
                        <button key={label} type="button" onClick={() => setForm({ ...form, isInterState: v })} disabled={form.gstRate === 0}
                          className={`flex-1 py-2 text-xs font-medium transition ${form.isInterState === v ? 'bg-white text-black hover:bg-gray-200' : 'bg-[#F1F5F9] text-[#64748B] hover:text-[#0F172A]'} disabled:opacity-50`}>
                          {label}
                        </button>
                      ))}
                    </div>
