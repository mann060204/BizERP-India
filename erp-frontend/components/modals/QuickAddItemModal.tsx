...
const emptyForm = {
  ...
  gstRate: '', cessRate: 0, igstRate: 0, saleDiscount: 0, saleDiscountType: 'percentage',
...
const Input = ({ label, required = false, type = 'text', keyName, form, setForm, placeholder = '', onQuickAdd }: any) => (
  <div>
    ...
    <input type={type}
      ...
      className="w-full px-3 py-2 rounded-lg bg-yellow-50 border border-slate-200 text-slate-900 placeholder-[#475569] focus:outline-none focus:border-[#D4D4D4] text-sm transition" />
  </div>
);
...
const Select = ({ label, required = false, keyName, form, setForm, options, onQuickAdd }: any) => (
  <div>
    ...
    <select value={form[keyName]} onChange={e => setForm({ ...form, [keyName]: e.target.value })}
      className="w-full px-3 py-2 rounded-lg bg-yellow-50 border border-slate-200 text-slate-900 focus:outline-none focus:border-[#D4D4D4] text-sm transition appearance-none">
...
const Checkbox = ({ label, keyName, form, setForm, danger = false }: any) => (
  <label className={`flex items-center gap-2 text-sm cursor-pointer ${danger ? 'text-red-400 font-medium' : 'text-slate-900'}`}>
    <input type="checkbox" checked={form[keyName]} onChange={e => setForm({ ...form, [keyName]: e.target.checked })}
      className="w-4 h-4 rounded border-slate-200 bg-yellow-50 text-action-500 focus:ring-action-400 focus:ring-offset-black" />
    {label}
  </label>
);
...
>
          <div className="bg-yellow-50 border border-slate-200 rounded-2xl w-full max-w-6xl shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-center justify-between p-5 border-b border-slate-200 shrink-0">
              ...
              <button onClick={() => onClose()} className="p-2 rounded-xl hover:bg-yellow-50 text-slate-600 hover:text-slate-900 transition"><X className="w-5 h-5" /></button>
            </div>
              ...
              <div className="flex-1 p-5 overflow-y-auto bg-white">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="border border-slate-200 rounded-xl p-4 bg-yellow-50">
        ...
        >
          <div className="bg-yellow-50 text-slate-900 border border-slate-200 w-full max-w-[440px] flex flex-col shadow-2xl rounded-2xl overflow-hidden">
            ...
            <div className="flex items-center justify-between p-5 border-b border-slate-200 bg-white">
              ...
              <button onClick={() => setShowUnitModal(false)} className="p-2 rounded-xl hover:bg-yellow-50 text-slate-600 hover:text-slate-900 transition"><X className="w-5 h-5" /></button>
            </div>
            ...
            <div className="p-6 space-y-6 bg-yellow-50">
              ...
              <div className="p-4 rounded-xl border border-[#1e3a8a]/30 bg-white">
                 <div className="flex justify-between items-center mb-3">
                   ...
                   <div className="text-[11px] text-blue-400 font-semibold bg-action-500/10 px-2 py-1 rounded-md border border-action-400/20">1 {form.unit || 'Main Unit'} = {form.conversionRate || 1} {form.secondaryUnit || 'Second Unit'}</div>
                 </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-6">
                  <label className="flex items-center gap-2 text-sm text-slate-900 cursor-pointer group">
                    <input type="radio" checked={form.secSalePriceType !== 'margin'} onChange={() => setForm({...form, secSalePriceType: 'fixed'})} className="w-4 h-4 rounded-full border-slate-200 bg-yellow-50 text-action-500 focus:ring-action-400 focus:ring-offset-black" />
                    Fixed Per Unit
                  </label>
                  <label className="flex items-center gap-2 text-sm text-slate-600 cursor-pointer hover:text-slate-900 transition group">
                    <input type="radio" checked={form.secSalePriceType === 'margin'} onChange={() => setForm({...form, secSalePriceType: 'margin'})} className="w-4 h-4 rounded-full border-slate-200 bg-yellow-50 text-action-500 focus:ring-action-400 focus:ring-offset-black" />
                    Margin Per Unit
                  </label>
                </div>
                <div className="flex rounded-lg overflow-hidden border border-slate-200 focus-within:border-[#D4D4D4] transition">
                  <div className="bg-yellow-50 text-slate-600 px-4 py-2.5 border-r border-slate-200 flex items-center justify-center text-sm font-medium">₹</div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="flex rounded-lg overflow-hidden border border-slate-200 focus-within:border-[#D4D4D4] transition">
                    <div className="bg-yellow-50 text-slate-600 px-3 py-2.5 border-r border-slate-200 flex items-center justify-center text-sm">₹</div>
              ...
              <label className="flex items-center gap-3 text-sm text-slate-600 cursor-pointer hover:text-slate-900 transition pt-2">
                <input type="checkbox" checked={form.isDefaultSecondaryUnit || false} onChange={e => setForm({...form, isDefaultSecondaryUnit: e.target.checked})} className="w-4 h-4 rounded border-slate-200 bg-yellow-50 text-action-500 focus:ring-action-400 focus:ring-offset-black" />
